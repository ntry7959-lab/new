...
exports.smsg = (nted, m, store) => {
    ...
    if (m.key) {
        ...
        m.sender = nted.decodeJid(m.fromMe && nted.user.id || m.participant || m.key.participant || m.chat || '')
    ...
    if (m.message) {
		...
		* @returns 
	   */
            m.quoted.copyNForward = (jid, forceForward = false, options = {}) => nted.copyNForward(jid, vM, forceForward, options)
	...
	*/
    m.reply = (text, chatId = m.chat, options = {}) => Buffer.isBuffer(text) ? nted.sendMedia(chatId, text, 'file', '', m, { ...options }) : nted.sendText(chatId, text, m, { ...options })
    /**
	* Copy this message
	*/
	 ...
	 * @returns 
	 */
	m.copyNForward = (jid = m.chat, forceForward = false, options = {}) => nted.copyNForward(jid, m, forceForward, options)

nted.appenTextMessage = async(text, chatUpdate) => {
let messages = await generateWAMessage(m.chat, { text: text, mentions: m.mentionedJid }, {
userJid: nted.user.id,
quoted: m.quoted && m.quoted.fakeObj
})
messages.key.fromMe = areJidsSameUser(m.sender, nted.user.id)
...
nted.ev.emit('messages.upsert', msg)
}
