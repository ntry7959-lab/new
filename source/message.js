...
async function LoadDataBase(Fyzz, m) {
	try {
...
async function MessagesUpsert(Fyzz, message, store) {
	try {
		let botNumber = await Fyzz.decodeJid(Fyzz.user.id);
		const msg = message.messages[0];
		const type = msg.message ? (getContentType(msg.message) || Object.keys(msg.message)[0]) : '';
		
		if (msg.key && msg.key.remoteJid === 'status@broadcast') {
		if (global.db.settings.readsw && global.db.settings.readsw == true) {
		Fyzz.readMessages([msg.key])
		} else return
		}		
		if (!msg.message) return
		if (!Fyzz.public && !msg.key.fromMe && message.type === 'notify') return
		if (global.db.settings.autoread && global.db.settings.autoread == true) Fyzz.readMessages([msg.key])
		if (global.db.settings.autotyping && global.db.settings.autotyping == true && !msg.key.fromMe) Fyzz.sendPresenceUpdate('composing', msg.key.remoteJid)
		const m = await Serialize(Fyzz, msg, store)
		if (m.isBaileys) return
		require('../Lucax.js')(Fyzz, m, message, store);
		if (type === 'interactiveResponseMessage' && m.quoted && m.quoted.fromMe) {
			let apb = await generateWAMessage(m.chat, { text: JSON.parse(m.msg.nativeFlowResponseMessage.paramsJson).id, mentions: m.mentionedJid }, {
				userJid: Fyzz.user.id,
				quoted: m.quoted
			});
			apb.key = msg.key
			apb.key.fromMe = areJidsSameUser(m.sender, Fyzz.user.id);
			...
			Fyzz.ev.emit('messages.upsert', pbr);
		}
	} catch (e) {
		throw e;
	}
...
async function Solving(Fyzz, store) {
	Fyzz.public = true
	
	Fyzz.serializeM = (m) => MessagesUpsert(Fyzz, m, store)
	...
	Fyzz.getName = (jid, withoutContact  = false) => {
		const id = Fyzz.decodeJid(jid);
		if (id.endsWith('@g.us')) {
			const groupInfo = store.contacts[id] || Fyzz.groupMetadata(id) || {};
	...
	Fyzz.sendContactV2 = async (jid, kon, desk = "Developer Bot", quoted = '', opts = {}) => {
let list = []
for (let i of kon) {
list.push({
...
Fyzz.sendMessage(jid, { contacts: { displayName: `${list.length} Kontak`, contacts: list }, ...opts }, { quoted })
}
	
	Fyzz.sendContact = async (jid, kon, quoted = '', opts = {}) => {
		let list = []
		for (let i of kon) {
			list.push({
				displayName: await Fyzz.getName(i + '@s.whatsapp.net'),
				vcard: `BEGIN:VCARD\nVERSION:3.0\nN:${await Fyzz.getName(i + '@s.whatsapp.net')}\nFN:${await Fyzz.getName(i + '@s.whatsapp.net')}\nitem1.TEL;waid=${i}:${i}\nitem1.X-ABLabel:Ponsel\nitem2.ADR:;;Indonesia;;;;\nitem2.X-ABLabel:Region\nEND:VCARD` //vcard: `BEGIN:VCARD\nVERSION:3.0\nN:${await Fyzz.getName(i + '@s.whatsapp.net')}\nFN:${await Fyzz.getName(i + '@s.whatsapp.net')}\nitem1.TEL;waid=${i}:${i}\nitem1.X-ABLabel:Ponsel\nitem2.EMAIL;type=INTERNET:whatsapp@gmail.com\nitem2.X-ABLabel:Email\nitem3.URL:https://instagram.com/Fyzz_dev\nitem3.X-ABLabel:Instagram\nitem4.ADR:;;Indonesia;;;;\nitem4.X-ABLabel:Region\nEND:VCARD`
			})
		}
		Fyzz.sendMessage(jid, { contacts: { displayName: `${list.length} Kontak`, contacts: list }, ...opts }, { quoted })
	}
	...
	Fyzz.profilePictureUrl = async (jid, type = 'image', timeoutMs) => {
		const result = await Fyzz.query({
			tag: 'iq',
			attrs: {
	...
	Fyzz.sendFileUrl = async (jid, url, caption, quoted, options = {}) => {
		async function getFileUrl(res, mime) {
			if (mime && mime.includes('gif')) {
				return Fyzz.sendMessage(jid, { video: res.data, caption: caption, gifPlayback: true, ...options }, { quoted });
			} else if (mime && mime === 'application/pdf') {
				return Fyzz.sendMessage(jid, { document: res.data, mimetype: 'application/pdf', caption: caption, ...options }, { quoted });
			} else if (mime && mime.includes('image')) {
				return Fyzz.sendMessage(jid, { image: res.data, caption: caption, ...options }, { quoted });
			} else if (mime && mime.includes('video')) {
				return Fyzz.sendMessage(jid, { video: res.data, caption: caption, mimetype: 'video/mp4', ...options }, { quoted });
			} else if (mime && mime.includes('audio')) {
				return Fyzz.sendMessage(jid, { audio: res.data, mimetype: 'audio/mpeg', ...options }, { quoted });
			}
		}
	...
	Fyzz.sendTextMentions = async (jid, text, quoted, options = {}) => Fyzz.sendMessage(jid, { text: text, mentions: [...text.matchAll(/@(\d{0,16})/g)].map(v => v[1] + '@s.whatsapp.net'), ...options }, { quoted })
	
	Fyzz.sendAsSticker = async (jid, path, quoted, options = {}) => {
	 ...
	 if (options && (options.packname || options.author)) {
		...
		await Fyzz.sendMessage(jid, { sticker: { url: buffer }, ...options }, { quoted });
		return buff;
	}
	
	
	    Fyzz.sendVideoAsSticker = async (jid, path, quoted, options = {}) => {
	...
	Fyzz.downloadAndSaveMediaMessage = async (message, filename, attachExtension = true) => {
	...
	Fyzz.sendMedia = async (jid, path, fileName = '', caption = '', quoted = '', options = {}) => {
		const { mime, data, filename } = await Fyzz.getFile(path, true);
		...
		await Fyzz.sendMessage(jid, { [type]: { url: pathFile }, caption, mimetype, fileName, ...options }, { quoted, ...options });
...
async function Serialize(Fyzz, m, store) {
	const botNumber = await Fyzz.decodeJid(Fyzz.user.id)
	...
	if (m.key) {
		...
		m.sender = await Fyzz.decodeJid(m.fromMe && Fyzz.user.id || m.participant || m.key.participant || m.chat || '')
		if (m.isGroup) {
			m.metadata = m.isGroup ? (await Fyzz.groupMetadata(m.chat).catch(_ => [{}]) || [{}]) : [{}]
	...
	if (m.message) {
		...
		if (m.quoted) {
			...
			m.quoted.sender = Fyzz.decodeJid(m.msg.contextInfo.participant)
			m.quoted.fromMe = m.quoted.sender === Fyzz.decodeJid(Fyzz.user.id)
			...
			m.getQuotedObj = async () => {
				...
				let q = await store.loadMessage(m.chat, m.quoted.id, Fyzz)
				return await Serialize(Fyzz, q, store)
			}
			m.quoted.key = {
				remoteJid: m.msg?.contextInfo?.remoteJid || m.chat,
				participant: m.quoted.sender,
				fromMe: areJidsSameUser(Fyzz.decodeJid(m.msg?.contextInfo?.participant), Fyzz.decodeJid(Fyzz?.user?.id)),
			...
			m.quoted.delete = () => {
				Fyzz.sendMessage(m.quoted.chat, {
					delete: {
						remoteJid: m.quoted.chat,
	...
	m.copy = () => Serialize(Fyzz, proto.WebMessageInfo.fromObject(proto.WebMessageInfo.toObject(m)))
		...
		try {
			if (/^https?:\/\//.test(text)) {
				...
				if (/gif|image|video|audio|pdf/i.test(mime)) {
					return Fyzz.sendFileUrl(chatId, text, caption, quoted, options)
				} else {
					return Fyzz.sendMessage(chatId, { text: text, mentions: [...text.matchAll(/@(\d{0,16})/g)].map(v => v[1] + '@s.whatsapp.net'), ...options }, { quoted })
				}
			} else {
				return Fyzz.sendMessage(chatId, { text: text, mentions: [...text.matchAll(/@(\d{0,16})/g)].map(v => v[1] + '@s.whatsapp.net'), ...options }, { quoted })
			}
