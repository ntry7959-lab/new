...
const rsz = async (image, ukur1 = 100, ukur2 = 100) => {
	return new Promise(async(resolve, reject) => {
		try {
...
async function brat(text) {
    ...
    const url = `https://fgsi-brat.hf.space/?text=${encodeURIComponent(text)}&isVideo=false`;
    const res = await axios.get(url, { responseType: 'arraybuffer' });
    return res.data; // Buffer hasil gambar/stiker
}

async function bratVideo(text) {
    if (!text) throw 'Teks tidak boleh kosong';
    const url = `https://fgsi-brat.hf.space/?text=${encodeURIComponent(text)}&isVideo=true`;
    const res = await axios.get(url, { responseType: 'arraybuffer' });
    return res.data; // Buffer hasil video
}
...
module.exports = {
  ...
  remini,
  mediafire,
  tiktokDl,   // diperbaiki dari `tiktokDI`
  brat,
  bratVideo,
  rsz
};